...
import { db } from '@/lib/firebase'; // Re-added Firebase import
import { doc, updateDoc } from 'firebase/firestore'; // Re-added Firebase import
...
const BookingForm: React.FC<BookingFormProps> = ({ onBack, onNext, onDataUpdate, formData, currentUser }) => {
  ...
  const validateAddress = (): boolean => {
    ...
    if (!addressState.trim()) {
      ...
      isValid = false;
    } else if (addressState.trim().length !== 3) { // Ensure 3 characters for state abbreviation
      setStateError("Enter a valid 3-character state abbreviation (e.g., NSW, VIC).");
      isValid = false;
    }
    if (!postcode.trim()) {
  ...
  if (!currentUser) {
    // This case should ideally be handled by parent page redirecting to login
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p>No user signed in. Redirecting...</p>
      </div>
    );
  }
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gray-200 p-6 rounded-lg">
            ...
            {isEditingInstructions ? (
              ...
              <>
                <div 
                  className="bg-white p-4 rounded-lg mb-4 cursor-pointer hover:bg-gray-50 transition-colors duration-150 ease-in-out" 
