...
const DescribeIssue: React.FC<DescribeIssueProps> = ({ onNext, onDataUpdate, formData }) => {
  ...
  // Condition to determine if there are any previous details to show
  const hasPreviousAddress = formData.streetNumber && formData.streetName && formData.suburb && formData.state && formData.postcode;
  const hasPreviousInstructions = formData.arrivalInstructions && formData.arrivalInstructions.trim() !== "";
  const showPreviousDetails = hasPreviousAddress || hasPreviousInstructions;
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          {showPreviousDetails && (
            <div className="mb-6 p-4 bg-gray-50 rounded-lg">
              <h3 className="text-sm font-semibold mb-2">Previously Entered Details:</h3>
              {hasPreviousAddress && (
              ...
              {hasPreviousInstructions && (
                <p className="text-gray-700"><span className="font-medium">Arrival Instructions:</span> "{formData.arrivalInstructions}"</p>
              )}
            </div>
          )}
