...
const Jobs = () => {
  ...
  const handleCancelBooking = async (job: Job) => {
    ...
    if (!job.bookingId || !job.jobId) {
        setError("Cannot cancel: Missing booking or job ID.");
        return;
    }

    if (job.status === 'Completed' || job.status === 'Cancelled') {
        setError(`This job is already ${job.status.toLowerCase()} and cannot be cancelled again.`);
        return;
    }
    ...
    try {
      const functions = getFunctions(app);
      const cancelBookingFn = httpsCallable<CancelBookingPayload, CancelBookingResult>(functions, 'cancelBooking');
      
      await cancelBookingFn({ bookingId: job.bookingId, jobId: job.jobId });
