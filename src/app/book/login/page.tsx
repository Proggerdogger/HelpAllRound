...
export default function LoginPage() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        // User is signed in (could be anonymous or phone authenticated).
        // Only redirect if not in the middle of an OTP verification process
        if (!showOtpInput && !loading) {
          console.log(`User already authenticated (UID: ${user.uid}, Anonymous: ${user.isAnonymous}), redirecting to /book/choice`);
      ...
      setCheckingAuthState(false); // Auth state determined
    });

    return () => unsubscribe(); // Cleanup subscription on unmount
  }, [router, showOtpInput, loading]); // Add showOtpInput and loading to prevent redirect during OTP flow
