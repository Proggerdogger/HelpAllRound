...
import { useAuth } from '@/contexts/AuthContext'; // Use the central AuthContext
...
export default function LoginPage() {
  ...
  const { currentUser, loadingAuthState } = useAuth(); // Use context for auth state
  ...
  // Effect for checking auth state from context and redirecting if already logged in
  useEffect(() => {
    // Wait until the auth state is resolved from the context
    if (loadingAuthState) {
      return; // Do nothing while loading
    }
    
    if (currentUser && !currentUser.isAnonymous) {
      // If a non-anonymous user is logged in, and we are not in the middle of an OTP flow, redirect.
      if (!showOtpInput && !loading) {
        console.log(`User already authenticated (UID: ${currentUser.uid}, Anonymous: false), redirecting to /book/choice`);
        router.push('/book/choice');
      }
    } else if (currentUser && currentUser.isAnonymous) {
      // If user is anonymous, let them stay on the login page.
      console.log(`User is anonymous (UID: ${currentUser.uid}). Login page will remain accessible.`);
    } else {
      // User is not authenticated (currentUser is null).
      console.log("User is not authenticated, login page will remain accessible.");
    }
  }, [currentUser, loadingAuthState, router, showOtpInput, loading]);

  // This function is now only responsible for CREATING the instance.
  const createRecaptcha = useCallback(async (): Promise<RecaptchaVerifier | null> => {
    if (typeof window === 'undefined') return null;

    // Aggressively clean up any prior instance to prevent rendering errors.
    ...
    try {
      ...
      // The render() method is async and must be awaited.
      await verifier.render();
      window.recaptchaVerifier = verifier; // Assign to window ONLY after render succeeds.
      return verifier;
    } catch (e) {
      console.error("[LoginPage] createRecaptcha: Error during creation/rendering.", e);
      setError("Failed to initialize reCAPTCHA. Please try again.");
      return null;
    }
  }, []);

  // Minimal effect for cleanup ONLY when the component is removed from the screen.
  useEffect(() => {
    return () => {
      if (window.recaptchaVerifier) {
        window.recaptchaVerifier.clear();
  ...
  const handleSubmitPhoneNumber = async () => {
    ...
    if (!verifier) {
      setLoading(true);
      console.log("[LoginPage] handleSubmit: No verifier found, creating one now.");
      const newVerifier = await createRecaptcha();
      setLoading(false);
        ...
        {!showOtpInput ? (
          <>
            ...
            <button
              ...
              disabled={loading || loadingAuthState}
