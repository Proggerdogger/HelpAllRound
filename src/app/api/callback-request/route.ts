...
if (!admin.apps.length) {
  try {
    if (serviceAccountKeyPath) {
      // The `require` statement is necessary here for dynamically loading the key file
      // based on an environment variable path, which standard ES6 imports cannot do.
